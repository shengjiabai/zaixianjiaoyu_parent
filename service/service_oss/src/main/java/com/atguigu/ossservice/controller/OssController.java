...
import com.atguigu.ossservice.clienttest.DueContrroer;
...
public class OssController {
    ...
    @Autowired
    private DueContrroer dueContrroer;
    ...
    //上传文件到阿里云OSS（比如上传讲师头像、上传课程封面等都是调用此方法）
    @ApiOperation(value = "test6666")
    @PostMapping("uploadFileTo")//文件上传时提交的数据以多段的形式进行拼接，最终数据以二进制流的形式发送给服务器
    public R uploadFileTo(String file){//获取上传的文件，spring mvc文件上传就这种操作(*￣︶￣)
        R test = dueContrroer.test(file);
        return test;
    }
